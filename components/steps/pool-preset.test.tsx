...
describe('PoolStep preset shelf handoff', () => {
  ...
  it('sends preset=true on the first batches, then stops after 3 accepted games', async () => {
    ...
    // Accept the 3 visible slots, then drive one more decision once a fresh replacement is
    // interactive. Each decision pops one from the backlog; the 4th drains it below REFILL_AT and
    // triggers a refill fetch. Clicks on a still-exiting (already-decided) card are safe no-ops,
    // so retrying inside waitFor handles the AnimatePresence exit timing.
    fireEvent.click(playedButtons[0]);
    fireEvent.click(playedButtons[1]);
    fireEvent.click(playedButtons[2]);

    // After 3 accepts, the post-handoff fetch must NOT include preset (personalization takes over).
    await waitFor(() => {
  ...
  it('keeps preset=true if the user only passes (no accepts), until the shelf drains', async () => {
    ...
    // AnimatePresence exit timing (clicks on already-decided cards are safe no-ops).
